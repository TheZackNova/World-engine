...
- relation: 该势力对{{user}}的态度——"血盟"/"盟友"/"友好"/"中立"/"冷淡"/"紧张"/"敌对"/"世仇"。
  血盟=绝对信任，生死与共；盟友=地位平等，互相支援；友好=认同{{user}}，优先合作；中立=不关心不排斥；冷淡=已注意到但不打算采取行动；紧张=接近冲突边缘；敌对=公开对抗；世仇=不死不休。
- currentGoal: 当前目标文字
- core_person: 核心人物姓名
...
### reputation（声誉对象）
四维声誉，每维六级：天怒人怨→声名狼藉→默默无闻→小有名气→受人尊敬→万众敬仰
...
推演时按以下因果顺序检查：
1. 先将所有持续中的天下大势作为本轮世界级约束，并检查是否形成新大势或已有大势明确结束。
2. 判断本轮事实、行动与公开信息是否形成新风声。
3. 检查已有风声是否获得新的合法传播节点，并据此更新 level/scope/content/source。
4. 判断风声实际覆盖了哪些势力、圈层或行动者；只有被覆盖者才能据此改变判断与行动。
5. 天下大势或风声造成跨系统变化时，在对应状态字段中落实结果，并用 influenceChain 记录传导过程。
6. 不得从面板全知信息直接跳到 NPC 行动，不得为了产生联动而虚构传播节点。
