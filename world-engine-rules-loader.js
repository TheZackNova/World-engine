...
冲突型 threshold = CONFLICT_STAGE_BASE[stage] - 200*r*(1-r) - (level-1)*10，maxFails = 6 - level。
推进型 threshold = PROGRESS_STAGE_BASE[stage] - 200*r*(1-r) + (level-1)*10，maxFails = 2 + level。
其中 r = min(1, stageRound/9)。

判定：dice > threshold → 成功推进（stageRound+1）；dice < threshold*0.4 → 受挫倒退（stageRound-1）；其余 → 保持。
本地骰子只负责正常推进和终局晋级（已爆发/已完成）。已消散/已失败由推演API根据世界状态直接判定。
...
六、势力之间的关系
用固定词表描述势力间的关系状态，仅限使用以下8个层级词：血盟、盟友、友好、中立、冷淡、紧张、敌对、世仇。禁用层级外模糊词。
...
二、触发机制
本地骰子每轮固定3%概率触发区域突发事件。触发后冷却10轮。不绑定地区稳定度、不绑定世界紧张度、不绑定事件链等级、不做保底——天灾人祸本就具有偶然性。
本地骰子负责：判定是否触发、决定事件类型。
后台API负责：根据指定类型，生成具体事件标题、发生地点、影响范围、传播风声、外溢影响。
...
世界运转：
- 世界非中心化。NPC有自己的生活目标、日程和社交圈，不会无事围着{{user}}转。
- 事件链、风声、团体进度即使与{{user}}无关也会自动推进。
- 每轮对话世界向前推进一步。
- 事件链分为冲突型 conflict（萌芽→发酵→逼近→已爆发，可由AI判定已消散）和推进型 progress（筹备→执行→关键→已完成，可由AI判定已失败）。
- 事件 type 一旦确定不得改动；同名事件更新必须沿用原 type。若完成事项引发冲突或冲突引出善后工程，应新建另一条事件链，并在影响链中记录传导关系。
- 停滞表示仍可能恢复；已消散/已失败表示确定无法继续，属于不可逆终局，必须有明确因果。
- 影响链只记录跨系统外溢变化，使用 trigger（触发源）→ impact（直接影响）→ fallout（后续余波）三段结构。
- NPC即使不在{{user}}视线内也在过自己的生活，每轮应有NPC独立行动的体现。

事件链：
- 冲突型：Lv表示冲突烈度，Lv越高越容易推进，maxFails=6-Lv。
- 推进型：Lv表示完成难度，Lv越高越难推进，maxFails=2+Lv。
- 每个阶段内部通过骰子系统驱动进度。本地骰子只负责正常推进和终局晋级。已消散/已失败由推演API根据世界状态直接判定。
- 事件链停滞≠放弃，执行方转入低烈度外围准备状态。
- 特权修正：受害者地位越高，事件级别跃升越严重。

势力：
- 团体至少3个，每个势力必须包含 name/scope/status/relation/currentGoal/core_person。
- status 为整体运势：鼎盛→稳固→倾轧→困顿→衰落→瓦解。
- relation 为对{{user}}态度，八级：血盟→盟友→友好→中立→冷淡→紧张→敌对→世仇。
- core_person 有1-3个权力支柱（powerPillars，仅名称字符串），可被事件链逐个摧毁。所有支柱被摧毁后核心人物失去权力地位。
- 势力层面的态度对立（relation）≠仇敌，不可互相转化。

天下大势：
- 天下大势是长期、广域、跨系统并迫使多个势力持续调整行动的世界级局势。
- 每轮检查 Lv4 事件终局与 Lv4 风声等候选来源，但不得机械升级；持续中的大势每轮都作为世界约束。
- 天下大势不参与骰子、不自动消散，只有明确结束时才能标记为已结束。

风声：
- 每条包含 topic/type/level/content/scope/source；同 topic 覆盖更新，禁止强制凑数。
- 只有风声覆盖相关对象后才允许改变其判断和行动；跨系统变化必须写入 influenceChain。
- 连续多轮没有实质更新的风声会被本地消散骰直接删除；仍在传播或持续影响世界时，必须返回相同 topic 的更新。
...
区域突发事件：
- 本地骰子每轮3%概率触发区域级突发事件（山贼劫道、大火、洪涝、疫病、饥荒等），触发后冷却10轮。
- 事件与{{user}}的行为链无关，来自世界本身的混乱。禁止写成已有势力策划的阴谋。
- 本地骰子决定是否触发和事件类型；API根据指定类型生成具体事件、风声和影响链。

经济：
- 经济脉搏是世界的血液循环，不是{{user}}的个人账本。
- climate（"繁荣"/"平稳"/"衰退"/"动荡"）表示当前区域经济温度。
- signals 记录市场中值得注意的变化，每项 { summary, scope }，不超过3条。必须由事件链或外部原因驱动，不能凭空波动。
- 重大经济变化产生经济风声，公众行动可反过来影响经济并与事件链联动。

声誉：
- 四维声誉（朝堂/市井/草莽/同道）独立升降，不同圈子的人看对应的维度。
- 草莽之中：体制外力量对{{user}}的看法。核心标准是有种还是没种——以私力对抗体制不公、替弱者出头则加分；欺压平民、出卖同道则减分。单纯刑事犯罪不自动获得草莽尊重。
- 行为形成覆盖对应圈层的风声后，才允许改变该圈层声誉。

信息黑盒：
- 黑盒分为隐秘行为（做过的事）和隐秘资产（握在手里的资源）。未在知情名单中的NPC必须彻底不知情。
- 隐秘行为追痕迹（目击者/物证），隐秘资产追暴露度（0-100）和可用性（有效/过期/暴露/失效）。
- 禁绝上帝视角，不知情的NPC不能有任何暗示、怀疑或"第六感"。
    `.trim();
